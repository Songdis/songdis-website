...
import { MOCK_ANALYTICS, VIEW_OPTIONS, type AnalyticsView } from "@/app/mock/analytics"; // MOCK_ANALYTICS used only for stat card fallbacks while loading
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <StatCard label="Total Streams"
            value={isLoading ? "..." : (s?.totalStreams.value ?? "0")}
            sub={s?.totalStreams.sub ?? "All DSPs"}
            icon={mock.totalStreams.icon} highlight
            badge={s?.totalStreams.change || undefined} />
          <StatCard label="Avg / Day"
            value={isLoading ? "..." : (s?.avgPerDay.value ?? "0")}
            sub={s?.avgPerDay.sub ?? "Per day"}
            icon={mock.avgPerDay.icon} />
          <StatCard label="Releases"
            value={isLoading ? "..." : (s?.releases.value ?? "0")}
            sub={s?.releases.sub ?? "With data"}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <StatCard label="Countries"
            value={isLoading ? "..." : (s?.countries.value ?? "0")}
            sub={s?.countries.sub ?? "Territories"}
            icon={mock.countries.icon} />
          <StatCard label="Platforms"
            value={isLoading ? "..." : (s?.platforms.value ?? "0")}
            sub={s?.platforms.sub ?? "DSPs"}
            icon={mock.platforms.icon} />
          <StatCard label="Playlist"
            value={isLoading ? "..." : (s?.playlists.value ?? "0")}
            sub={s?.playlists.sub ?? "Active"}
        ...
        <div className="flex items-center gap-2 rounded-xl border border-dashed overflow-x-auto border-[#C30100]/30 bg-[#0E0808] px-4 py-3">
          {PERIODS.map((p) => (
