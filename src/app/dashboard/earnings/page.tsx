...
export default function EarningsPage() {
  ...
  const { splitEarnings, revenueByPlatform, withdrawalInfo } = MOCK_EARNINGS;
    ...
    <DashboardLayout customCta={{ label: "Withdraw Funds", onClick: () => setWithdrawOpen(true) }}>
      <div className="flex flex-col gap-5">
        ...
        <div className="rounded-2xl border border-dashed border-[#C30100]/30 bg-[#180F0F] p-5">
          ...
          {splitEarnings.releases.map((release) => (
            <div key={release.id} className="rounded-xl bg-[#0E0808] border border-white/[0.06] p-4 mb-3 last:mb-0">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="relative w-12 h-12 rounded-lg overflow-hidden shrink-0">
                    <Image src={release.cover} alt={release.title} fill className="object-cover" unoptimized />
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={release.breakdown} dataKey="split" cx="50%" cy="50%" innerRadius={14} outerRadius={22} strokeWidth={0}>
                          {release.breakdown.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                        </Pie>
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
              ...
              <div className="border-t border-white/[0.05] pt-3">
                ...
                {release.breakdown.map((collab) => (
                  <div key={collab.name} className="grid grid-cols-3 gap-2 py-1.5">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: collab.color }} />
                      <span className="font-body text-white text-xs">{collab.name} {collab.isYou && <span className="text-white/40">(You)</span>}</span>
                    </div>
                    <p className="font-body text-white/60 text-xs text-right">{collab.role}</p>
                    <p className="font-body text-white/60 text-xs text-right">{collab.split}%</p>
                  </div>
