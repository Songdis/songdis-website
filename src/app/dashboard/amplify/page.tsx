...
export default function AmplifyPage() {
  ...
  /* Build the widget src — append JWT if token exists */
  const token = getToken();
  const src = `${WIDGET_BASE}?app-key=${APP_KEY}&autodetect=1${token ? `&jwt=${token}` : ""}`;
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col gap-5">
        ...
        {/* Widget container */}
        <div className="rounded-2xl border border-white/[0.06] bg-[#180F0F] overflow-hidden relative">
          {/* Loading skeleton */}
          {!loaded && (
