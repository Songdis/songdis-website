...
//           <div key={c.email} className="grid grid-cols-3 gap-2 py-1.5">
...
function SplitAgreementForm({
  ...
  const handleSubmit = () => {
    onSubmit({
      ...
      splitName:
        (splitName ||
          musicUploadOptions.find((m) => m.id === selectedUploadId)?.title) ??
        "Split Agreement",
      collaborators,
    });
  };
        ...
        <div className="p-8">
          ...
          <div className="flex flex-col gap-5">
            ...
            {collaborators.map((collab, i) => (
              <div key={i} className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="font-body text-white/70 text-xs">
                    Collaborator Email
                  </label>
                  <input
                    type="email"
            ...
            {/* Note */}
            <div className="border border-dashed border-[#C30100]/30 rounded-xl px-4 py-3">
              <p className="font-body text-white/60 text-xs font-semibold mb-1">
                NOTE:
              </p>
              <p className="font-body text-white/40 text-xs leading-relaxed">
                All parties get an email to accept split. Payment is distributed
                automatically during each payout cycle.
              </p>
            </div>
    ...
    <div className="rounded-xl bg-[#0E0808] border border-white/[0.06] p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10 rounded-lg overflow-hidden shrink-0">
            <Image
              src={split.trackCover}
              alt={split.trackTitle}
              fill
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={{
              color: "#C30100",
              backgroundColor: "rgba(195,1,0,0.10)",
              border: "1px solid rgba(195,1,0,0.25)",
            }}
          >
            <TrashIcon /> Delete
          </button>
      ...
      <div className="border-t border-white/[0.05] pt-3">
        <div className="hidden sm:grid grid-cols-3 gap-2 mb-2">
          <p className="font-body text-white/30 text-[10px] uppercase tracking-wider">
            Collaborators
          </p>
          <p className="font-body text-white/30 text-[10px] uppercase tracking-wider text-right">
            Role
          </p>
          <p className="font-body text-white/30 text-[10px] uppercase tracking-wider text-right">
            Split %
        ...
        {split.collaborators.map((c) => (
          <div
            key={c.email}
            className="flex flex-wrap sm:grid sm:grid-cols-3 gap-1 sm:gap-2 py-1.5"
          >
            <span className="font-body text-white text-xs truncate">
              {c.name} {c.isYou && <span className="text-white/40">(You)</span>}
            </span>
            <span className="font-body text-white/60 text-xs text-right">
              {c.role}
            </span>
            <span className="font-body text-white/60 text-xs text-right">
              {c.split}%
...
export default function SplitrPage() {
  ...
  const musicUploadOptions = releases.map((r) => ({
    id: Number(r.id),
    title: r.title,
  }));
  ...
  const handleUpdate = async (data: {
    ...
    // Step 2 — update existing recipients' percentages
    const existingRecipients = activeSplit.collaborators.filter(
      (c) => !c.isYou,
    );
    for (const collab of data.collaborators) {
      const existing = existingRecipients.find((r) => r.email === collab.email);
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5">
          ...
          {[
            {
              ...
              value: isLoading ? "..." : stats.activeAgreements,
              icon: "/images/money.svg",
              highlight: true,
            },
            {
              label: "Collaborators",
              value: isLoading ? "..." : stats.collaborators,
              icon: "/images/streams.svg",
            },
            {
              label: "Pending Acceptance",
              value: isLoading ? "..." : stats.pendingAcceptance,
              icon: "/images/streams.svg",
            },
          ].map((s) => (
            <div
        ...
        {/* Active Splits */}
        <div className="rounded-2xl border border-dashed border-[#C30100]/30 bg-[#180F0F] p-5">
          <p className="font-body text-white text-sm font-medium mb-4">
            Active Splits
          </p>
          {isLoading ? (
        ...
        {vizSplit && (
          <div className="rounded-2xl border border-dashed border-[#C30100]/30 bg-[#180F0F] p-5">
            <p className="font-body text-white text-sm font-medium mb-5">
              Split Visualization
            </p>
...
const selectCls =
  "w-full appearance-none bg-[#0E0808] border border-white/10 rounded-lg px-4 py-3 font-body text-white text-sm outline-none focus:border-[#C30100] transition-colors pr-8";
