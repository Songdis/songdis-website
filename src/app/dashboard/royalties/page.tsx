...
export default function RoyaltyReportPage() {
  ...
  const { data, isLoading } = useRoyalties();
...
function exportCSV(data: RoyaltiesPageData | null) {
  if (!data) return;
  const rows: string[][] = [];
  rows.push([`Royalty Report`]);
  rows.push([]);
...
function exportPDF(data: RoyaltiesPageData | null) {
  if (!data) return;
  ...
  </head><body><h1>Royalty Report</h1>
  <div class="sg">
  ...
  <tbody>${data.monthlyTrends.map((m) => `<tr><td>${formatMonth(m.month)}</td><td>$${m.earnings.toFixed(2)}</td><td>${m.streams.toLocaleString()}</td><td>$${m.avgRate.toFixed(6)}</td></tr>`).join("")}</tbody></table>
  </body></html>`;
