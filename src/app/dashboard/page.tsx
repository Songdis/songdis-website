...
//                 // DES-003: cards are now clickable links to the music page
//                 <Link
//                   key={String(release.id)}
//                   href="/dashboard/music"
//                   className="relative flex-1 aspect-[3/4] rounded-xl overflow-hidden group cursor-pointer"
//                 >
//                   {/* DES-002: cover uses album_art_url from API, fallback to placeholder */}
//                   {release.cover ? (
//                     <Image
//                       src={release.cover}
//                       alt={release.title}
...
export default function DashboardPage() {
  ...
  return (
    ...
    <DashboardLayout pageTitle="Splitr" showWelcome>
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-2xl border border-white/[0.06] bg-[#180F0F] p-5 flex flex-col">
            ...
            {/* DES-002: recent_releases from dashboard-stats with album_art_url */}
            <div className="flex gap-3">
              {recentReleases.length === 0 ? (
                <p className="font-body text-white/30 text-sm py-6">No releases yet.</p>
              ) : recentReleases.map((release) => (
                <Link
                  key={String(release.id)}
                  href="/dashboard/music"
                  className="relative flex-1 aspect-[3/4] rounded-xl overflow-hidden group cursor-pointer min-w-0"
                >
                  {release.cover ? (
                    <Image
                      src={release.cover}
