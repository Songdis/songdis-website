...
function ResetPasswordForm() {
   ...
   *
   * `otp` is still read as a fallback so a link generated by the older flow
   * keeps working while any are still in the wild.
   */
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!email || !token) {
      setFieldErrors({
        newPassword: "This reset link is incomplete. Please request a new one from the sign-in page.",
      });
      return;
    }
    ...
    mutate(
      { email, otp: token, newPassword, confirmPassword },
      () => setShowSuccess(true)
    );
  };
