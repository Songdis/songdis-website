      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="border border-dashed border-[#C30100]/25 rounded-xl p-5">
          ...
          <label
            ...
            onClick={() => update({ isPreviouslyReleased: !state.isPreviouslyReleased })}
          >
            <div className={[
              "w-4 h-4 rounded border flex items-center justify-center transition-colors",
              state.isPreviouslyReleased ? "border-[#C30100] bg-[#C30100]" : "border-white/20 group-hover:border-white/40",
            ].join(" ")}>
              {state.isPreviouslyReleased && <CheckIcon />}
            </div>
            <div>
          ...
          {/* Original Release Date (if previously released) */}
          {state.isPreviouslyReleased && (
            <div className="mb-5">
              <label className="font-body text-white/70 text-xs block mb-1.5">Original Release Date</label>
              <input
                type="date"
              ...
              />
              <p className="font-body text-white/30 text-[11px] mt-1">When was this content originally released?</p>
            </div>
          )}
        ...
        <div className="border border-dashed border-[#C30100]/25 rounded-xl p-5">
          ...
          {/* DSP grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-4 gap-x-4 mb-4">
            {filtered.map((dsp) => {
          ...
          {fieldErrors.platforms && <p className="font-body text-[#C30100] text-xs mt-2 text-center">{fieldErrors.platforms}</p>}
        ...
        <div className="border border-white/[0.06] rounded-xl p-4">
          ...
          {[
            { label: "Release Type", value: state.releaseType === "single" ? "Single" : "Album/EP" },
