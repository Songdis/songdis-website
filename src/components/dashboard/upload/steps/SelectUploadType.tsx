...
export default function SelectUploadType({ selected, onSelect, onContinue }: Props) {
  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      {/* Stack one per row on mobile/tablet, three across from sm up */}
        ...
        {TYPES.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => onSelect(t.id)}
              className={[
                "flex sm:flex-col items-center sm:text-center text-left gap-4 sm:gap-0 p-4 sm:p-6 rounded-xl border transition-all duration-200 focus-visible:outline-none",
                isSelected
                  ? "border-[#C30100] bg-[#C30100]/10"
                  : "border-white/[0.08] bg-white/[0.03] hover:border-white/20",
              ].join(" ")}
