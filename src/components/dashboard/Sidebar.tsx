...
function LogoutModal({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-[480px] rounded-2xl bg-[#140C0C] border border-white/[0.07] p-8">
        {/* Close */}
        <button
          onClick={onCancel}
        ...
        <h2 className="font-heading text-white uppercase text-xl tracking-wide mb-5">Log Out</h2>

        {/* Info box */}
        <div className="rounded-xl bg-[#C30100]/10 border border-[#C30100]/20 px-4 py-3 mb-7">
          <p className="font-body text-white text-sm">Are you sure you want to log out?</p>
  ...
  return (
    <li className="relative group/item">
      ...
      >
        ...
        >
          <Image src={item.icon} alt={item.label} fill className="object-contain" unoptimized />
        </span>
        {!collapsed && <span className="font-body text-sm truncate">{item.label}</span>}
      </Link>
      ...
      {collapsed && (
        <div className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-3 z-50 opacity-0 group-hover/item:opacity-100 transition-opacity duration-150">
          <div className="relative">
            <div
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1.5 w-0 h-0"
  ...
  return (
    <div>
      {collapsed ? (
        ...
        <p className="font-heading text-white/30 uppercase text-[10px] tracking-[0.25em] px-3 mb-2">{label}</p>
      )}
    ...
    <>
      ...
      >
        ...
        >
          ...
          {collapsed && (
            <Link href="/dashboard" className="relative shrink-0 flex items-center justify-center ml-3">
          ...
          {collapsed ? (
            <div className="relative group/user">
              ...
              <div className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-3 z-50 opacity-0 group-hover/user:opacity-100 transition-opacity duration-150">
                <div className="bg-[#1A0808] border border-white/[0.08] rounded-lg px-3 py-2 shadow-xl">
                  <p className="font-heading text-white text-xs uppercase tracking-wide whitespace-nowrap">{MOCK_USER.name}</p>
                  <p className="font-body text-white/40 text-[10px] whitespace-nowrap">{MOCK_USER.plan}</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              ...
              <div className="min-w-0">
                <p className="font-heading text-white text-xs uppercase tracking-wide truncate">{MOCK_USER.name}</p>
                <p className="font-body text-white/40 text-[10px] truncate">{MOCK_USER.plan}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Logout confirmation modal — rendered outside aside so it covers the full viewport */}
      {showLogoutModal && (
        <LogoutModal
